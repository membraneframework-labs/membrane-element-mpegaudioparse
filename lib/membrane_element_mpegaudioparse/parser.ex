defmodule Membrane.Element.MPEGAudioParse.Parser do
  ...
  defp do_parse(
    ...
    with {:invalid_frame, false} <-
           {:invalid_frame,
            caps |> Map.values() |> Enum.any?(fn val -> val in [:invalid, :free] end)},
  ...
  defp parse_crc_enabled(<<1::size(1)>>), do: true
  defp parse_crc_enabled(<<0::size(1)>>), do: false
  ...
  defp parse_padding_enabled(<<1::size(1)>>), do: true
  defp parse_padding_enabled(<<0::size(1)>>), do: false

  defp parse_private(<<1::size(1)>>), do: true
  defp parse_private(<<0::size(1)>>), do: false
  ...
  defp parse_original(<<1::size(1)>>), do: true
  defp parse_original(<<0::size(1)>>), do: false

  defp parse_copyright(<<1::size(1)>>), do: true
  defp parse_copyright(<<0::size(1)>>), do: false
